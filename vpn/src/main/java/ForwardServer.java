...
public class ForwardServer {
    ...
    private static final String programPath = System.getProperty("user.dir") + "/";
    ...
    private void doHandshake() throws Exception {
        ...
        if (clientHello.getParameter("MessageType").equals("ClientHello")) {
            ...
            //CertificateHandler.verifyCertificate(CertificateHandler.getCertificate((programPath + CA)), clientCert);
            CertificateHandler.verifyCertificate(CertificateHandler.getCertificate(CA), clientCert);

            // Send serverHello message
            X509Certificate serverCert = CertificateHandler.getCertificate(serverCertificate);
            HandshakeMessage serverHello = new HandshakeMessage();
            serverHello.putParameter("MessageType", "ServerHello");
            serverHello.putParameter("Certificate", CertificateHandler.encodeCertificate(serverCert));
        ...
        // setting the target port and host sent by the client
        Handshake.targetHost = forward.getParameter("TargetHost");
        Handshake.targetPort = (Integer.parseInt(forward.getParameter("TargetPort")));
        ...
        byte[] encryptedBytesKey = SessionKey.encryptSessionKey(sessionKey, clientPubKey);
        ...
        byte[] encryptedBytesIV = SessionIV.encryptSessionIV(sessionIV, clientPubKey);
    ...
    public static void main(String[] args)
        ...
        System.out.println(CA + " ca " + serverCertificate + " server cert");
